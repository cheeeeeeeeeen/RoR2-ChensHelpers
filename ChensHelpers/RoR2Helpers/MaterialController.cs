...
namespace Chen.Helpers.RoR2Helpers
{
    ...
    public class MaterialController : MonoBehaviour
    {
        private void Start()
        {
            Log.Warning($"Attached Material Controller to {gameObject.name}. Ignore this if you are a mod developer debugging. Report otherwise.");
            ...
            foreach (var renderer in renderers)
            {
                ...
                if (material)
                {
                    ...
                    HGBaseController controller = null;
                    switch (material.shader.name)
                    {
                        case "Hopoo Games/Deferred/Standard":
                            controller = rendererGameObject.AddComponent<HGStandardController>();
                            break;

                        case "Hopoo Games/FX/Cloud Remap":
                            controller = rendererGameObject.AddComponent<HGCloudRemapController>();
                            break;

                        case "Hopoo Games/FX/Cloud Intersection Remap":
                            controller = rendererGameObject.AddComponent<HGIntersectionController>();
                            break;
                    }
                    if (controller)
                    {
                        RecursivePathGeneration(rendererGameObject);
                        controller.Material = material;
                        controller.Renderer = renderer;
                        Log.Warning($"{controller.GetType().Name} assigned to {rendererGameObject.name}.");
                    }
                    else Log.Error($"Shader Controller not implemented for {material.shader.name}.");
                }
            }
            Destroy(this);
        }
        ...
        {
            if (parent) rendererGameObject.name = $"{parent.gameObject.name}.{rendererGameObject.name}";
            if (!parent) parent = rendererGameObject.transform.parent;
...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
    ...
    public class HGBaseController : MonoBehaviour
    {
        ...
        public void SetShaderKeywordBasedOnBool(bool enabled, string keyword)
        {
            if (!Material)
            {
                Log.Error($"Material field was null, cannot run shader keyword method.");
                return;
            }
